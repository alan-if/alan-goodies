...
----- This library file defines common verbs needed in gameplay.
----- The verbs are listed alphabetically. This file also includes common
----- commands which are not actually verbs, such as "inventory", "verbose" and
----- "again". You are free to edit this file for your own purposes in any way
----- you like by adding, deleting or modifying verbs. Verbs originally defined
...
--| yes              |                                      | yes                               |   0   |     |
--|------------------+--------------------------------------+-----------------------------------+-------+-----+
--| --- Verbs having to do with wearing clothes are defined in the file 'classes.i', subclass 'clothing'. --- |
--| --- These verbs are:                                                                                  --- |
--|------------------+--------------------------------------+-----------------------------------+-------+-----+
...
----- the use of verbs with objects and actors in an irrational way. For example,
----- the hero won't be able to eat anything that is not edible, and so forth.
...
VERB 'about'
  DOES
    ...
    "This is a text adventure, also called interactive fiction, which means that
     what goes on in the story depends on what you type at the prompt.
     Commands you can type are for example GO NORTH (or NORTH or just N), WEST,
     ...
     LOOK (L) describes your surroundings, and INVENTORY (I) lists what you are
     carrying. You can SAVE your game and RESTORE it later on.
...
ADD TO EVERY ACTOR
  ...
  AND act IS reachable -- you might want to remove this check in some situations,
                       -- e.g. when an NPC (= a non-player character) is speaking
...
VERB brief
  DOES
    Visits 1000.
    "Brief mode is now on. Location descriptions
     will only be shown the first time you visit."
END VERB.
...
SYNTAX consult_error = consult (source)
  WHERE source ISA THING
    ELSE "To consult something, please use the formulation
          CONSULT PERSON/THING ABOUT PERSON/THING."
...
VERB credits
  DOES
    "The author retains the copyright to this game.
     ...
     $nE-mail address: thomas.nilsson@progindus.se
     $pFurther information about the ALAN system can be
     obtained from the World Wide Web Internet site
...
-- The verbs 'empty' and 'pour' have similar syntaxes and behaviour here.
-- They are, however, not declared as synonyms but kept separate, as their usage
-- doesn't overlap 100%; for example you can pour liquids but not empty them.
-- That's why in 'classes.i', liquids are defined only to work with the verb
-- 'pour', and the verb 'empty' is disabled for liquids.
...
ADD TO EVERY THING
  VERB follow
    ...
    AND act NOT NEAR hero -- this check presumes that if the actor to be followed
                          -- is in an adjacent location, the hero will be able to
                          -- follow him/her.
      ELSE "You follow" SAY THE act. "."
        LOCATE hero AT act.
    DOES
      "You don't quite know where" SAY THE act. "went.
       You must state a direction where you want to go."
  END VERB.
END ADD TO.
...
SYNTAX go_to = 'to' (dest)!   -- because 'go' is predefined in the parser,
  WHERE dest ISA THING        -- it can't be used in verb definitions
        ...
        THEN
          IF dest IS NOT reachable
            ...
            ELSE
              ...
              END IF.
          END IF.
        ELSE "You can't see" SAY THE dest. "anywhere nearby.
              You must state a direction where you want to go."
      END IF.
  END VERB.
END ADD TO.
...
SYNONYMS walk = go. -- here we define a synonym for the predefined parser word
                    -- 'go' which is not visible in the syntax itself.
                    -- Thus, you will be able to say e.g. both 'go to shop' and
                    -- 'walk to shop' (as well as e.g. both 'go east' and 'walk
                    -- east').
...
VERB hint
  DOES "Unfortunately hints are not available in this game."
END VERB.
...
VERB i
  DOES
    ...
    -- to be listed after 'inventory', you should manually add the listing right
    -- here according to the following example:
    -- ~~~~~~~~~~~~~~~~
    -- IF bag IN hero
    --   THEN LIST bag.
    -- END IF.
    -- ~~~~~~~~~~~~~~~~
    --
    -- This way, 'inventory' will yield e.g. "You are carrying a bag. The bag
    -- contains a book." If you leave the above addition out, the outcome will
    -- be just "You are carrying a bag.", with no comment on what is inside the
    -- bag.
...
-- the response will be "It will be difficult to attack anything while lying down."
  ...
  VERB lie_in
    ...
    DOES
      "There's no need to lie down in" SAY THE cont. "."
        -- If you need this to work, insert the following two lines instead of
        -- the above:
...
ADD TO EVERY OBJECT
  VERB look_out_of
    CHECK obj IS examinable
      ELSE "You can't look out of" SAY THE obj. "."
    AND CURRENT LOCATION IS lit
      ELSE "It is too dark to see."
    DOES
      IF obj IS NOT plural
...
SYNTAX notify = notify.
       ...
       notify_on = notify 'on'.   -- The instructions tell the player that mere
                                  -- 'notify' is enough, but these two verbs are
                                  -- implemented
       notify_off = notify 'off'. -- In case (s)he adds the prepositions to the
                                  -- end anyway.
...
-- The following event is run each turn to check if the game score is greater
-- than the last recorded score (which is stored in the Hero's 'oldscore'
-- attribute). If the score is greater, then the 'Score has gone up...' text is
-- displayed (as long as the player hasn't disabled it by using the 'notify'
-- verb - which sets 'notify_on' to off -- i.e. the hero 'IS NOT notify_on'.)
...
ADD TO EVERY OBJECT
  VERB play_with
    CHECK CURRENT LOCATION IS lit
      ELSE "It is too dark to see."
    DOES
      "After second thoughts you don't find it purposeful
       to start playing with" SAY THE obj. "."
  END VERB.
END ADD TO.
    ...
    WHEN obj
      ...
      AND obj IS reachable
        ELSE SAY THE obj.
          ...
          "out of your reach."
      DOES
        "Using" SAY THE instr. "you push" SAY THE obj.
  ...
  VERB put_against
    WHEN obj
      ...
      DOES
        ...
        -- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        -- (+ you would need probably need an attribute to
        -- check that the objectis leaning against the bulk)
  END VERB.
END ADD TO.
  ...
  VERB put_near, put_behind, put_under
    WHEN obj
      ...
      DOES
        ...
        --   THEN  "(taking" SAY THE obj. "first)$n"
        -- END IF.
        -- "You put" SAY THE obj. "near" -- (or behind or under)
        -- SAY THE bulk. "."
        -- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        -- (+ you would need to define some attributes to check
        -- that the object is behind another object, etc.)
  END VERB.
END ADD TO.
...
ADD TO EVERY THING
  VERB 'say'
    DOES "You utter """ SAY topic. "$$"". Nothing happens."
  END VERB.
END ADD TO.
...
VERB 'score'
  DOES SCORE.
    -- (or, if you wish to disable the score, use the following
    -- kind of line instead of the above:)
...
VERB 'script'
  DOES
    "You can turn transcripting on and off using the 'script on/off' command
     within the game. The transcript will be available in a file with a name
     starting with the game name.
   $pIn a GUI version you can also find this in the drop-down menu in the
     interpreter.
   $pIn a command line version you can start your game with the '-s' switch
     to get a transcript of the whole game."
END VERB.
...
VERB shout
  DOES "Nothing results from your $ving."
END VERB.
...
VERB smell0
  DOES "You smell nothing unusual."
END VERB.
...
ADD TO EVERY OBJECT
  VERB take
    ...
    AND obj NOT DIRECTLY IN worn
      -- i.e. the object to be taken is a piece of clothing that the player
      -- character is wearing; here, this verb works in practise like 'take off'.
  ...
  VERB take_from
    WHEN obj
      ...
      AND obj IN holder
        ELSE
          IF obj ISA CLOTHING
          -- A piece of clothing worn by an NPC is not *in* the NPC but in a
          -- special npc_worn container (see 'classes.i', class 'actor').
          -- We need to take the class 'clothing' separately into account here.
            THEN
              -- Here, we allow the piece of clothing to be taken if it is
              -- takeable, skipping the remaining checks below.
        ...
        IF target IS inanimate
          THEN
            IF target NOT DIRECTLY AT hero    -- e.g. the target is inside a box
              THEN "It wouldn't accomplish anything trying to throw something at"
                SAY THE target. "."
              ELSE
                SAY THE projectile.
                IF projectile IS NOT plural
...
ADD TO EVERY OBJECT
  VERB tie
    DOES "You must state where do you want to tie" SAY THE obj. "."
  END VERB.
END ADD TO.
...
ADD TO EVERY THING
  VERB touch
    ...
    DOES "You feel nothing unexpected."
  END VERB.
END ADD TO.
    ...
    WHEN obj
      ...
      AND obj IS reachable
        ELSE SAY THE obj.
          ...
          "out of your reach."
      DOES
        "You touch" SAY THE obj. "with" SAY THE instr.
        ". Nothing special happens."
  END VERB.
END ADD TO.
    ...
    ELSE
      ...
      "something you can turn."
       ...
       turn = rotate (obj). -- we don't declare 'rotate' a synonym for 'turn'
                            -- through a SYNONYMS statement as we don't want it
                            -- to be possible for the player to type something
                            -- like 'rotate tv on' (see 'turn on' and 'turn off'
                            -- below)
...
----- defined in 'classes.i' with proper checks for 'on' and 'NOT on', 'lit' and
----- 'NOT lit'.
----- Trying to turn on or off an ordinary object will default here to "That's
----- not something you can turn on".
...
-- A separate 'switch' verb is declared in 'classes.i', classes 'device' and
-- 'lightsource'.
-- This verb merely covers cases where the player forgets (or doesn't bother) to
-- type 'on' or 'off'.
...
VERB verbose
  DOES
    VISITS 0.
    "Verbose mode is now on. Location descriptions
     will be always shown in full."
END VERB.
