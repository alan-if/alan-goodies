...
-- play.
--
-- This file also defines three specific location classes:
--     rooms (= indoor locations),
--     sites (= outdoor locations) as well as the class 'dark_location'.
--
-- Finally, the attributes 'visited' and 'described' as well as the instance
-- 'room' (to make possible player commands such as 'examine room' and 'exit
-- room') are defined.

-- You may modify this file in any way that suits your purposes.
-- To use this file, you should have it in the same folder as your source code
-- file, and the line
--
--    IMPORT 'locations.i'.
--
...
--    hardwired to the interpreter in any way, so you can replace them
--    altogether or add new ones to be used alongside with them.
...
-- NOTE: it is often a good idea to modify the 'examine' verb for the above
...
--   VERB examine DOES ONLY
--     IF hero AT kitchen
--       THEN "Numerous shelves, full of various kitchen
--             utensils, line the walls."
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--
-- In other words, making the description of the walls, etc., individual for
-- each room gives a better impression than if you just use the default messages.
...
-- (Edit the description in the code above to change this.)
...
-- In darkness, you are not able to manipulate things other than turn on a
-- lightsource and drop items you're carrying (these checks are found in
-- 'verbs.i'). You can exit normally and use verbs that don't require seeing,
-- such as 'smell', 'listen' and 'think'.

-- If you are in a dark location with an NPC (= a non-player character), you are
-- able to communicate with him by asking and telling but not by showing and
-- giving. If you wish to change this, see the respective verbs in 'verbs.i' and
-- edit their checks.


-- Note that you cannot change the name of a location mid-game. Thus, if you
-- define a dark location called e.g. 'Darkness' and wish to make it lit at some
-- point in the game, the name will still be 'Darkness' even if the location
-- description can be changed to describe the illuminated location.

-- To change the location name, you must locate the hero in another location
-- when the dark location is lit. For example,
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
-- Suppose you want all dark_locations in the game to become lighted.
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--
-- FOR EACH makes a loop that affects all instances of the class described after
-- the ISA statement. The 'd1' above is a temporary variable needed in the FOR
-- EACH phrase and not anywhere else in the game code. You can name this
-- variable in any way you wish.
...
-- time, and the value increases on every subsequent visit.
-- This helps when you need to control if or how many times a location has been
-- visited, and also if you want the location description to be different after
-- the first visit.
...
-- This distinction is handy when you want the first-time description of a
-- location to be different from the subsequent ones (even if the hero is in the
-- location still for the first time).
  ...
  ENTERED
     IF CURRENT ACTOR = hero
      THEN
        ...
        INCREASE described OF THIS.   -- this value also increases after 'look'
                                      -- (see 'verbs.i'.).
     END IF.
...
-- A location not visited at all has the 'visited' value 0. When the hero enters
-- it the first time, the 'visited' value will change to 1. On the second visit
...
-- Note that if you have an NPC moving around in the game, the visited value of
-- any location will increase when the NPC enters the location, as well (ENTERED
-- applies to all moving actors). This is most often not what is wanted, and
-- that's why the 'if' statement (IF CURRENT ACTOR = hero) is included above.
...
--             ELSE """Just take what you want from the chamber"",
--                   the king smiles."
--          END IF.
--       ...
--       END IF.
...
-- Suppose you want the location description to be different after the first
-- time the description is shown, even if you are in the location still for the
-- first time. Then, you can use the 'described' attribute, e.g.:
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
-- Through defining the instance 'room' we make possible player commands such as
-- 'examine room' and 'exit room', among others. These commands only work in
-- indoor locations.
--
-- We have to identify this instance differently from 'room' which has already
-- been defined as a class above, that's why we identify it here as
-- 'current_room'.
--
-- Through the NAME statement, it can be still called a 'room'in-game.
