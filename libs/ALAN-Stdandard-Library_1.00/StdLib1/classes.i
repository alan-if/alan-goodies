...
-- BACKGROUND
  ...
  -- A typical example of a background would be for example the sun, or a
  -- mountain visible in the distance.
...
-- CLOTHING
  ...
  -- extension. The said extension has been fully assimilated to this library.
  -- This extension prevents clothes from being worn in an illogical order, e.g.
  -- you cannot put on a shirt if you are already wearing a jacket, and so
  -- forth. Also the verbs 'wear', 'remove' and 'undress' are defined here.
...
-- LIGHTSOURCE
  ...
  -- Can be turned on and off, lighted and extinguished (= put out) if it is not
...
-- LISTABLE_CONTAINER
  -- Is a special kind of container, the contents of which will be listed both
  -- after 'look' (= in the room description), 'look in' and 'examine' (if the
  -- container is open).
...
-- SCENERY
  ...
  -- The default message for 'take' will be "Unimportant for your purposes, you
  -- decide to leave the [object] where it is."
  -- The default message for 'examine' will be "The [object] doesn't appear
  -- particularly interesting."
  -- A scenery object is not automatically listed after the room description
  -- when you type 'look'. You have to include it manually in the room
  -- description.
...
-- SOUND
  ...
  -- Cannot initially be turned on or off, this has to be implemented manually
  -- by giving the sound the 'switchable' attribute.


-- SUPPORTER
  -- You can put things on this and you can stand on this. It is declared a
  -- container, so you can take things from it, as well. When there's something
  -- on a supporter, an automatic listing of it will appear in the room
  -- description and after 'examine'.
...
-- The ACTORS are defined here to be NOT inanimate CONTAINERS (so that they can
-- e.g. receive and carry things. The 'contents' (= possessions) of an actor
-- (except for the hero) are automatically listed after 'look' and 'examine'.
-- (For the hero, the command 'inventory' must be used.)
--
...
-- Note that a background object in ALAN3 is different from e.g. the backdrop in
-- Inform7 in that a background object is at one location at a time only, unless
...
-- Note that in this code you could define exits to other rooms of the house in
-- the normal way, without having to worry about the area at all, e.g.:
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- THE bedroom ISA LOCATION IN lamp_rooms
...
-- Naturally, you could also define a scenery object or a normal object to be in
-- several locations at once, in the same way. Note, however, that if you define
-- a takeable object in this manner, it will disappear from the other locations
-- once you take it in one location. Also, when manipulated (e.g. broken), the
-- object will be affected in all of the locations it is found.
...
-- To use this class, see the documentation text right after the code below.
-- This class makes use of Alan Bampton's 'xwear.i' extension written originally
-- for ALAN V2, converted here to V3 and assimilated fully to the present
-- library. Thanks to Alan Bampton for the permission to use the code here.
...
THE worn ISA ENTITY               -- an entity is present everywhere and thus
  CONTAINER TAKING CLOTHING.      -- the hero's clothing is always accessible
...
--------------------------------------------------------------------
      ...
      AND topcover OF THIS <= SUM OF topcover Directly IN worn
        THEN
...
--  IF THIS IN tempworn THEN
      ...
      AND handscover OF THIS <= SUM OF handscover Directly IN worn
        THEN
          INCREASE wear_flag OF hero BY 5.
      END IF.
      ...
      AND feetcover OF THIS <= SUM OF feetcover Directly IN worn
        THEN
          INCREASE wear_flag OF hero BY 5.
      END IF.
      ...
      AND headcover OF THIS <= SUM OF headcover Directly IN worn
        THEN
          INCREASE wear_flag OF hero BY 5.
      END IF.
...
--------------------------------------------------------------------
      ...
      IF tempcovered OF hero >31
      AND botcover OF THIS < 16
      AND botcover OF THIS <> 4
        THEN
...
--------------------------------------------------------------------
      ...
      IF  botcover OF THIS <> 0
      AND botcover OF THIS <= tempcovered OF hero
        THEN
          INCREASE wear_flag OF hero BY 5.
      END IF.
...
--------------------------------------------------------------------
      ...
      IF  topcover OF THIS <> 0
      AND topcover OF THIS < tempcovered OF hero
        THEN
...
--------------------------------------------------------------------
      ...
      IF  handscover OF THIS <> 0
      AND handscover OF THIS < tempcovered OF hero
        THEN
          INCREASE wear_flag OF hero BY 1.
      END IF.
      ...
      IF  feetcover OF THIS <> 0
      AND feetcover OF THIS < tempcovered OF hero
        THEN
          INCREASE wear_flag OF hero BY 1.
      END IF.
      ...
      IF  headcover OF THIS <> 0
      AND headcover OF THIS < tempcovered OF hero
        THEN
          INCREASE wear_flag OF hero BY 1.
      END IF.
      ...
      IF THIS IS NOT OPAQUE
        THEN
          IF COUNT ISA OBJECT, IN THIS > 0  -- if the piece of clothing contains
            THEN LIST THIS.                 -- something, e.g. if a jacket contains
          END IF.                           -- a wallet, the wallet will be mentioned
      END IF.                               -- when the jacket is examined
  END VERB.
...
-- THE jeans ISA CLOTHING IN wardrobe  -- IN wardrobe = in a container called
--   IS botcover 16.                   -- the wardrobe; the wardrobe is not a
-- END THE.                            -- separate location here.
...
--   IS topcover 16. -- Define separate containers like this for clothes worn
-- END THE.          -- by non-player characters.
                     -- If you defined here 'IN joe', the clothing would be
                     -- listed in Joe's possessions:
                     --     "You see Joe here. Joe is carrying
                     --      a book and a sweater."
                     -- For a concrete example of how to do this, scroll down
                     -- to the class 'actor'. Note also that if the piece of
                     -- clothing worn by an NPC is not meant to be takeable by
                     -- the player character, you should declare the the piece
                     -- of clothing to be NOT takeable.
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
--     Which attribute(s) to use depends on the type of clothing; see the
--     clothing table below.
--
...
--     table, don't mention this attribute in connection with your clothing
--     object.
...
-- The quick guide ends here. The clothing table follows right below. The text
-- following the table gives more details about the principles and the use of
-- this class.
...
-- Here is the chart showing a selection of fairly typical clothing items and
-- the values to set to obtain appropriate behaviour. Should you wish to create
-- an article of clothing not listed, usually a bit of lateral thought as to
-- what it is most like and where it fits into the scheme of things will suggest
-- a workable set of values, but be aware that you MUST use values in this
-- chart, simply adding things with intermediate values is probably going to
-- create nasty bugs:
...
-- The concept of "xwear.i"  (This and the following paragraphs are taken from
---------------------------   Alan Bampton's original  'xwear' documentation,
--                            with minor alterations.)


-- The basic idea behind this part of the library is that clothing is worn in
-- 'layers' and it is rather silly to allow players to (say) take off or put on
-- a shirt if they are wearing a jacket. To simulate this in ALAN I've chosen to
-- apply a numeric based layering system, and divide the body into five zones of
-- coverage. The zones are 'head', 'hands', 'feet', 'top' (for top half of
-- torso) and 'bot' (for bottom half of torso).

-- All objects have these five zones defined as default attributes (set to 0),
-- for the head zone the attribute is headcover for the hands zone handscover
-- and so on.

-- Every clothing object will thus need one or more of its 'zonecover'
-- attributes set to reflect the zone(s) it covers and its relative position in
-- the layers of clothing worn. A simple example would be a shirt, this covers
-- only the 'top' zone and so needs its topcover attribute to be set (to 8, just
-- why it's 8 will become clear shortly).

-- The principle used is that the closer to the skin an item is normally worn,
-- the lower its 'cover' attribute is. The library operates on the assumption
-- that items with higher value cover attributes for a particular zone are worn
-- over items with lower value attributes. When a player attempts to put on an
-- article of clothing, each zone it would affect is checked and compared to the
-- related zonal total of any clothes already worn. If the value of the new
-- clothing is not greater that the total(s) of clothing already worn (on a zone
-- by zone basis) then the library will not allow the wearing of that item.
-- There are a few notable exceptions to this rule, but I'll come back to those
-- a little later.
...
-- This part of the library might sound complex, but it is actually very simple
-- to use, here's an example of how it works.

-- Assume our hero starts the game wearing just vest and shorts and the player
-- issues the command 'put on shirt'. A quick check of the chart below should
-- reveal that a shirt has only its topcover attribute set to non-zero, (all the
-- other zones are zero, which means they are irrelevant for this item).

-- The library totals the topcover attributes of all the clothes currently worn,
-- like so:- Starting with a total of 0, and checking the vest, this has a
-- topcover attribute of 2, so total topcover is 0 + 2 = 2.

-- Next the library checks the shorts, these have a topcover attribute of 0, so
-- total topcover is 2 + 0 = 2.

-- As there are no other clothes to consider, the library now compares the
-- topcover attribute of the item we are attempting to put on, (a shirt with a
-- topcover attribute of 8 in this case) to the total value of items already
-- worn (2).

-- Because the topcover attribute of the shirt (8) is greater than the
-- calculated total (2), this is evaluated as being a 'legal' instruction and
-- the library allows the shirt to be put on.

-- Now consider the situation had the player started the game wearing vest,
-- shorts and a jacket, this is what happens should he try to 'put on the
-- shirt.'

-- Starting with a total of 0, and checking the vest, this has a topcover
-- attribute of 2, so total topcover is 0 + 2 = 2.

-- Next the library checks the shorts, these have a topcover attribute of 0, so
-- total topcover is 2 + 0 = 2.

-- Finally the library checks the jacket, this has a topcover attribute of 32,
-- so total topcover is 2 + 32 = 34.

-- Because the topcover attribute of the shirt (8) is now not greater than the
-- calculated total (34), this is evaluated as being an 'illegal' instruction
-- and the library won't allow the shirt to be put on.

-- That demonstrates the basic principle of the library, removing clothes uses a
-- variation of the 'compare to total' equation to allow / disallow removal of
-- clothing, an example would be that our vest, shorts and jacket wearing player
-- would NOT be allowed to remove the vest while he still had the jacket on.
...
-- Now I'll confuse the issue. Firstly the numbers in my chart are not born of
-- some sort of weird fixation with multiples, there is a very good reason why
-- the numbers are set as they are, computer/maths types will recognise the
-- sequence and realise it is all 'binary' based and know it makes it possible
-- to calculate exactly what the player is wearing in terms of layers.

-- Some female clothing breaks the rules defined above and is not so easy to
-- deal with. An example, although pantyhose is worn under a skirt, dress or
-- coat it can actually be put on or removed with the garment worn over it still
-- on. The library recognises this capability and deals with it properly by
-- assigning the dress/skirt and coat items particular properties in that they
-- don't affect the ability to wear or remove lower layer clothing that covers
-- the bottom of the torso only.

-- Although it's physically possible to put on/remove trousers while wearing a
-- skirt or dress, this (and a few other neat dressing/undressing tricks) is
...
-- That covers the way the library works, here's how to define objects to
-- actually make use of it. As an example, this is how to define a dress and
-- restrict it to being worn by a female.
...
-- Notice that it is only necessary to mention the attributes that are non-zero,
-- this minimises the amount of coding required. Apart from making sure the hero
-- is set as 'sex 2.' if we wish her to be able to wear this dress, this is all
-- the author needs to do, the library will police what is wearable and keep
-- track of things without any further work.

-- I expect most authors would prefer to start their games with a player at
-- least partially dressed. This is just a matter of defining the required
-- clothes object as being in the 'worn' container. Here's how to define
-- underwear that our female hero(ine?) starts the game wearing.
...
-- The library as it stands also prevents wearing of duplicate clothes, or
-- things that are logically mutually exclusive - e.g. the player can wear a
-- dress or a skirt, but not both.
...
EVERY liquid ISA OBJECT
  ...
  --   IN bottle
  --   HAS vessel bottle.  -- i.e. the value of the 'vessel' attribute
  --   ...                 -- is the container the liquid is in
  -- END THE juice.
  -- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  -- Then, taking and pouring liquids work smoothly.

  -- If you don't declare the 'vessel' attribute for the liquid instance, taking
  -- a liquid will yield "You can't carry [the liquid] around in your bare
  -- hands."; only taking its container will work in that case.
  ...
  VERB take
    DOES ONLY
      IF vessel OF THIS = no_vessel
      OR vessel OF THIS IS NOT takeable
    ...
    WHEN obj
      DOES ONLY
        ...
        IF THIS IN hero   -- i.e. if the implicit taking was successful
          THEN
            "You give" SAY THE vessel OF THIS.
            "to" SAY THE recip. "."
            LOCATE vessel OF THIS IN recip.
        END IF.

        -- there is no 'ELSE' statement in this last IF -clause, as the 'IF THIS
        -- NOT IN hero' clause above it takes care of the 'ELSE' alternative.
  ...
  VERB put_in
    ...
    WHEN cont
      DOES ONLY
        IF vessel OF THIS = no_vessel
          THEN
            "There's not much sense putting"
            SAY THE obj. "into" SAY THE THIS. "."
          ELSE
            IF vessel OF THIS IS NOT closed
              THEN
                IF obj = vessel OF THIS
                  THEN "That doesn't make sense."
                  ELSE "It wouldn't accomplish anything trying to put"
                    SAY THE obj. "into" SAY THE vessel OF THIS. "."
                END IF.
              ELSE "You can't, since" SAY THE vessel OF THIS.
                IF THIS IS NOT plural
                  THEN "is"
        ...
        IF THIS IN hero         -- i.e. if the implicit taking was successful
          THEN
            IF vessel OF THIS = no_vessel
              ...
              ELSE
                IF vessel OF THIS IS takeable
                  THEN "You put" SAY THE vessel OF THIS.
                       "onto" SAY THE surface. "."
                    LOCATE vessel OF THIS IN surface.
                  ELSE "You can't carry" SAY THE THIS.
                       "around in your bare hands."
                END IF.
            END IF.
        END IF.
    ...
    WHEN target
      DOES ONLY
        IF projectile = vessel OF THIS
          THEN "That doesn't make sense."
          ELSE "It wouldn't accomplish anything
                trying to throw" SAY THE projectile. "at"
            IF vessel OF THIS = no_vessel
              THEN SAY THE THIS.
              ELSE SAY THE vessel OF THIS.
            END IF.
  ...
  VERB throw_in
    WHEN projectile
      DOES ONLY
        "It wouldn't be sensible throwing"
        SAY THE THIS. "into" SAY THE cont. "."
    WHEN cont
      DOES ONLY
        IF projectile = vessel OF THIS
          THEN "That doesn't make sense."
          ELSE
            "It wouldn't be sensible trying to throw"
            SAY THE projectile. "in" SAY THE THIS. "."
        END IF.
  END VERB.
...
-- Note that closed listable_containers are by default opaque
-- and they become not opaque when they are opened:
...
--
-- The contents of a listable_container are also listed when the
-- container is opened. This doesn't happen with normal containers.


-- 2)


-- For the command 'inventory' to list the contents of a container the hero is
-- carrying, go to 'verbs.i', find the command 'inventory' and list one by one
-- all relevant containers that the hero might carry, thus:
--
-- ~~~~~~~~~~~~~~~~~~~~
-- VERB inventory
--   DOES LIST hero.
...
--
--    "You are carrying a bag and a box. The bag contains a loaf of bread.
--    The box is empty."
...
EVERY supporter ISA OBJECT
  ...
  -- in the following, we disable some verbs that are
  -- defined to work with normal containers:
  ...
  VERB put_in
    WHEN cont
      DOES ONLY
        "You can't put anything inside" SAY THE THIS. "."
  END VERB.
...
--   IN tray  -- note the IN here, even if the apple will
--   ...      -- be described as being on the tray
-- END THE.
--
-- THE book ISA OBJECT
--   IN table
...
--
--
-- Note that the 'examine' command will list what is on the surface of a
-- supporter, not what, if anything, is inside the supporter. For example, if
-- you have a supporter called 'table' in your game with two drawers in it,
--
-- DON'T do this:
-- ~~~~~~~~~~~~~~~~~~~~~~
...
--
--    "There's a table here. On the table you see a book, a bottom drawer
--     and a top drawer."
--
--
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--
-- In other words, declare the drawers components of the table, in the manner
-- described above. The result will then be e.g. something like this:
--
--    "You see a table here. There is a book on the table. The table has a
--     bottom drawer. The bottom drawer is closed. The table has a top drawer.
--     The top drawer contains a diary."
...
-- 'fire_with', 'kill_with', 'shoot' and 'shoot_with' where WEAPON
-- is used either in the syntax definitions or verb checks.)
...
-- When examined, a window is automatically described as being either open or
-- closed.
...
ADD TO EVERY ACTOR
  ...
  IS NOT sitting.
  IS NOT lying_down.
  IS NOT named.  -- = the actor's name is not known to the player;
                 --   see also the example below

  -- Important: if you don't need an article in front of an actor name (e.g.
  -- 'Jim', as opposed to e.g. 'a/the man'), declare the instance as 'named':
  ...
  -- (Remember also that all actors are 'CAN NOT talk' by default. If you want
  -- the actor to be able to talk, give it the attribute 'CAN talk' or declare
  -- it ISA PERSON (see further below))
  ...
  INDEFINITE ARTICLE
    IF THIS IS NOT named
      THEN
        IF THIS IS NOT plural
          THEN "a"  -- if you need "an", you must declare it
          ELSE ""   -- separately at the actor instance
        END IF.
      ELSE ""
    END IF.
  ...
  VERB examine
    DOES
      ...
      END IF.  -- Note that this doesn't apply to the hero which is defined separately further below.
               -- This listing will be overridden if you give an individual
               -- response to the actor instance being examined using DOES ONLY.
   END VERB.   -- If you want to still list the possessions of the actor after
               -- your own 'examine' response, you should state 'LIST [actor].'
               -- in the 'examine' verb there.
...
EVERY npc_worn ISA OBJECT
  HAS carrier no_carrier. -- The value of the 'carrier' attribute is the actor
                          -- wearing the clothing. Here, 'no_carrier' is a dummy
                          -- default instance that can be ignored.
...
-- To describe what an NPC is wearing (after the commands 'look and 'examine'),
-- define the NPC e.g. like this:
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- THE mr_smith ISA ACTOR
--  DESCRIPTION
--    "blah blah" LIST mr_smith_worn. -- Leave this LIST statement out if you
--                                    -- don't want to have the actor's clothing
--                                    -- listed after 'look'.
--    VERB examine
--    DOES ONLY "blah blah"
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--
-- Note that if you don't want the player character to be able to take a piece
-- of clothing worn by another character, you should declare the piece of
-- clothing NOT takeable!
...
-- TIP: if you have in your game an actor that starts off as unnamed (such as
--      'a man'), and the player learns his name later on (say, 'Jim'), you
--      should define the actor in e.g. the following way to make the player
--      able to refer to him with both 'man' and 'Jim':
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- THE jim ISA PERSON
--   NAME man NAME jim
--   PRONOUN him
...
-- The 'named' attribute is meant to be used in cases like this. The indefinite
-- and definite articles don't have to be declared here, as their behavior has
-- been declared in the general actor class above.
...
THE hero ISA ACTOR
  ...
  IS NOT seen_notify.   -- Records whether player has seen
                        -- the notify verb instructions yet.
  ...
  CONTAINER
    ...
    LIMITS  -- Remove this whole section (from this line until 'HEADER') if
            -- you wish to have no limits to how much the hero can carry.
      COUNT 10 Then
        "You can't carry anything more.
         You have to drop something first."
      WEIGHT 50 Then
        "You can't carry anything more.
         You have to drop something first."
    ...
    HEADER
      IF CURRENT LOCATION IS NOT lit
        THEN "Although you cannot see your belongings in the dark,
              you remember that you're carrying"
  ...
  VERB examine
    DOES ONLY
      "You notice nothing unusual about yourself."  -- edit this line to change the
                                                    -- way the hero is described
                                                    -- after the player command
                                                    -- 'examine myself'.
